...
set(ffmpeg_lnx_ext "tar.xz")
set(ffmpeg_lnx_url "https://github.com/BtbN/FFmpeg-Builds/releases/download/autobuild-2023-11-30-12-55/ffmpeg-N-112876-ga30adf9f96-linux64-gpl-shared.tar.xz")
set(ffmpeg_lnx_file "ffmpeg-N-112876-ga30adf9f96-linux64-gpl-shared")

set(FFMPEG_DIR  "${CMAKE_BINARY_DIR}/_deps/ffmpeg-install"  CACHE INTERNAL "")
#function for download ffmpeg
function(download_ffmpeg url file extn)
    file(DOWNLOAD "${url}" "${CMAKE_SOURCE_DIR}/${file}.${extn}")
    execute_process(COMMAND ${CMAKE_COMMAND} -E tar xf "${CMAKE_SOURCE_DIR}/${file}.${extn}" RESULT_VARIABLE result OUTPUT_QUIET)
    if(NOT result STREQUAL "0")
        message(FATAL_ERROR "Fetching and extract  ${file}.${extn}  failed!")
    endif()   
    file(REMOVE "${CMAKE_SOURCE_DIR}/${file}.${extn}")
...
IF (NOT "${FFMPEG_FOUND}")
    IF (WIN32)
        ...
        IF(NOT EXISTS "${FFMPEG_DIR}") 
            ...
            download_ffmpeg(${ffmpeg_url} ${ffmpeg_file} ${ffmpeg_ext})      
        ENDIF()
    ELSE()
        #try download ffmpeg linux-64 builds
        IF(NOT EXISTS "${FFMPEG_DIR}") 
            message("== FFmpeg linux-x64 not found. Downloading ...")
            download_ffmpeg(${ffmpeg_lnx_url} ${ffmpeg_lnx_file} ${ffmpeg_lnx_ext})      
        ENDIF()
    ENDIF()
